 ...
 */
@Stable
class SecureRelationalGroupedDataset protected[sql](
    private[sql] val df: SecureDataFrame,
    private[sql] val groupingExprs: Seq[Expression],
    groupType: SecureRelationalGroupedDataset.GroupType) {

  private[this] def toDF(aggExprs: Seq[Expression]): SecureDataFrame = {
    val aggregates = if (df.secureSparkSession.sessionState.conf.dataFrameRetainGroupColumns) {
    ...
    groupType match {
      case SecureRelationalGroupedDataset.GroupByType =>
        SecureDataset.ofRows(df.secureSparkSession, Aggregate(groupingExprs, aliasedAgg, df.logicalPlan))
      case SecureRelationalGroupedDataset.RollupType =>
        SecureDataset.ofRows(
          df.secureSparkSession, Aggregate(Seq(Rollup(groupingExprs)), aliasedAgg, df.logicalPlan))
      case SecureRelationalGroupedDataset.CubeType =>
        SecureDataset.ofRows(
          df.secureSparkSession, Aggregate(Seq(Cube(groupingExprs)), aliasedAgg, df.logicalPlan))
      case SecureRelationalGroupedDataset.PivotType(pivotCol, values) =>
        val aliasedGrps = groupingExprs.map(alias)
        SecureDataset.ofRows(
          df.secureSparkSession, Pivot(Some(aliasedGrps), pivotCol, values, aggExprs, df.logicalPlan))
    }
  }
   ...
   * @since 1.3.0
   */
   def agg(aggExpr: (String, String), aggExprs: (String, String)*): SecureDataFrame = {
   ...
   * @since 1.3.0
   */
  def agg(exprs: Map[String, String]): SecureDataFrame = {
   ...
   * @since 1.3.0
   */
   def agg(exprs: java.util.Map[String, String]): SecureDataFrame = {
  ...
  @scala.annotation.varargs
   def agg(expr: Column, exprs: Column*): SecureDataFrame = {
    toDF((expr +: exprs).map {
   ...
   * @since 1.3.0
   */
   def count(): SecureDataFrame = toDF(Seq(Alias(Count(Literal(1)).toAggregateExpression(), "count")()))
   ...
   */
  @scala.annotation.varargs
   def mean(colNames: String*): SecureDataFrame = {
   ...
   */
  @scala.annotation.varargs
  def max(colNames: String*): SecureDataFrame = {
   ...
   */
  @scala.annotation.varargs
   def avg(colNames: String*): SecureDataFrame = {
    aggregateNumericColumns(colNames : _*)(Average)
  }
   ...
   */
  @scala.annotation.varargs
   def min(colNames: String*): SecureDataFrame = {
   ...
   */
  @scala.annotation.varargs
   def sum(colNames: String*): SecureDataFrame = {
   ...
   * @since 1.6.0
   */
   def pivot(pivotColumn: String): SecureRelationalGroupedDataset = pivot(Column(pivotColumn))
   ...
   * @since 1.6.0
   */
   def pivot(pivotColumn: String, values: Seq[Any]): SecureRelationalGroupedDataset = {
   ...
   * @since 1.6.0
   */
   def pivot(pivotColumn: String, values: java.util.List[Any]): SecureRelationalGroupedDataset = {
    pivot(Column(pivotColumn), values)
  }
   ...
   * @since 2.4.0
   */
   def pivot(pivotColumn: Column): SecureRelationalGroupedDataset = {
    // This is to prevent unintended OOM errors when the number of distinct values is large
    val maxValues = df.secureSparkSession.sessionState.conf.dataFramePivotMaxValues
   ...
   * @since 2.4.0
   */
   def pivot(pivotColumn: Column, values: Seq[Any]): SecureRelationalGroupedDataset = {
    groupType match {
      case SecureRelationalGroupedDataset.GroupByType =>
        ...
        new SecureRelationalGroupedDataset(
          df,
          groupingExprs,
         SecureRelationalGroupedDataset.PivotType(pivotColumn.expr, valueExprs))
      case _: SecureRelationalGroupedDataset.PivotType =>
   ...
   * @since 2.4.0
   */
   def pivot(pivotColumn: Column, values: java.util.List[Any]): SecureRelationalGroupedDataset = {
   ...
   * workers.
   */
  private[sql] def flatMapGroupsInPandas(expr: PythonUDF): SecureDataFrame = {
    ...
    SecureDataset.ofRows(df.secureSparkSession, plan)
  }
  ...
  private[sql] def flatMapCoGroupsInPandas(
      r: SecureRelationalGroupedDataset,
      expr: PythonUDF): SecureDataFrame = {
...
private[sql] object SecureRelationalGroupedDataset {

  def apply(
      df: SecureDataFrame,
      groupingExprs: Seq[Expression],
      groupType: GroupType): SecureRelationalGroupedDataset = {
    new SecureRelationalGroupedDataset(df, groupingExprs, groupType: GroupType)
  }
